...
<x-app-layout>
    ...
    <div class="py-6 container mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        {{-- Alert Card for Expired Service Users --}}
        @if($usersWithServiceAlert->count() > 0)
        <div class="bg-red-600 text-white rounded-xl shadow-lg overflow-hidden">
            <div class="px-6 py-4 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <i class="fas fa-exclamation-triangle text-2xl"></i>
                    <h3 class="text-lg font-bold">Users with Expired Service</h3>
                </div>
                <span class="text-sm font-medium">{{ $usersWithServiceAlert->count() }} user(s)</span>
            </div>
            <div class="divide-y divide-red-500">
                @foreach($usersWithServiceAlert as $user)
                <div class="px-6 py-3 hover:bg-red-500 cursor-pointer transition-all duration-200">
                    <a href="{{ route('admin.users.index') }}" class="flex justify-between items-center">
                        <span>{{ $user->name }}</span>
                        <span class="font-semibold">{{ $user->last_service_date->format('Y-m-d') }}</span>
                    </a>
                </div>
                @endforeach
            </div>
